...
proxyToClash(std::vector<Proxy> &nodes, YAML::Node &yamlnode, const ProxyGroupConfigs &extra_proxy_group, bool clashR,
    ...
    for (const ProxyGroupConfig &x: extra_proxy_group) {
        ...
        std::cerr << "[DEBUG] Generating group - Name: '" << x.Name << "', Icon: '" << x.Icon << "', Icon empty: " << (x.Icon.empty() ? "true" : "false") << std::endl;
        
        if (!x.Icon.empty())
        {
            singlegroup["icon"] = x.Icon;
            std::cerr << "[DEBUG] Successfully added icon field: '" << x.Icon << "'" << std::endl;
        }
        else
        {
            std::cerr << "[DEBUG] Icon field is empty for group: '" << x.Name << "'" << std::endl;
        }
